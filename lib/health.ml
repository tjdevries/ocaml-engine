...
let draw t point =
  ...
  let rect = Rectangle.create (Vector2.x point) (Vector2.y point) cur_width height in
  Raylib.draw_rectangle_rec rect Color.green;
  let rect =
    Rectangle.create (cur_width +. Vector2.x point) (Vector2.y point) remaining height
  in
